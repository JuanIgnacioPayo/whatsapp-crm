    ...
    try {
        ...
        sock.ev.on('connection.update', async (update) => {
            ...
            if (connection === 'close') {
                ...
                console.log(`⚠️ Conexión de WhatsApp cerrada. Status: ${lastDisconnect?.error?.output?.statusCode}, Razón: ${lastDisconnect?.error?.message}, ¿Reconectar?:`, shouldReconnect);
                isConnected = false;
                connectedUser = null;
                try {
                    const io = (0, socket_service_1.getIO)();
            ...
            try {
                ...
                for (const msg of recentMessages) {
                    try {
                        ...
                        if (!fromJid)
                            continue;
                        if (ignoreGroups && fromJid.endsWith('@g.us'))
                            continue;
                        if (ignoreStatus && fromJid === 'status@broadcast')
                            continue;
        ...
        sock.ev.on('messages.upsert', async (m) => {
            ...
            for (const msg of m.messages) {
                ...
                if (ignoreGroups && fromJid.endsWith('@g.us'))
                    continue;
                if (ignoreStatus && (fromJid === 'status@broadcast' || fromJid.endsWith('@newsletter') || fromJid.endsWith('@broadcast')))
                    continue;
                const fromPhone = fromJid.replace('@s.whatsapp.net', '').replace('@c.us', '');
                const profileName = msg.pushName || undefined;
                let incomingText = '';
                ...
                try {
                    await (0, bot_service_1.handleIncomingMessage)(fromPhone, incomingText, 'WHATSAPP', profileName, msg.key?.id || undefined);
                }
                catch (err) {
